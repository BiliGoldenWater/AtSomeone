...
public class TabAtSomeone {
    ...
    TabCompleter atsomeone_tab = new TabCompleter() {
        ...
        public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
            ...
            switch (args.length){
                case 1:
                    if(CheckPermissions.hasPermissions(sender,"atsomeone.command.help") && "help".startsWith(args[0]))completions.add("help");
                    if(CheckPermissions.hasPermissions(sender,"atsomeone.command.ignore") && "ignore".startsWith(args[0]))completions.add("ignore");
                    if(CheckPermissions.hasPermissions(sender,"atsomeone.command.reload") && "reload".startsWith(args[0]))completions.add("reload");
                    return completions;
                case 2:
                    switch (args[0]){
                        case "ignore":
